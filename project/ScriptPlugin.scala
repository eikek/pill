...
object ScriptPlugin extends AutoPlugin {
  ...
  object autoImport {
    ...
    val `gen-pill` = taskKey[File]("Create the assembly and shell script")
    val `gen-zip` = taskKey[File]("Create a zip file containing pill.")
  }

  import autoImport._

  lazy val scriptSettings = Seq(
    shebang in script := "/usr/bin/env bash",
    assemblyDir in script := "./",
    javaBin in script := "java",
    javaOptions in script := Seq(),
    `gen-pill` in Compile := genBinImpl.value,
  ...
  def makeScript(sourceDir: File, classpath: String, shebang: String, java: String, javaOpts: String, cdtohere: Boolean = false): String = {
    val template = Template(sourceDir / "shell" / "pill")
    template.render(Map(
      "cdtohere" -> cdtohere,
      "shebang" -> shebang,
      "options" -> javaOpts,
  ...
  lazy val genBinImpl = Def.task {
    val template = sourceDirectory.value / "shell" / "pill"
    val out = target.value / "bin" / "pill"
    val body = makeScript(
  ...
  lazy val genZipImpl = Def.task {
    ...
    IO.write(zipDir / "pill", body)
    setExecutable(zipDir / "pill")
